    ...
    def forward(self, batch, mode='train', batch_idx=-1, constraints=None, cweights=None):
        ...
        for d, word_scores in enumerate(all_depth_scores):
            if mode == 'train':
                ...
                # ipdb.set_trace()
                # _, predictions = torch.max(word_log_probs[:,:,1:], dim=2)
                # predictions = predictions + 1

                # for rescoring using labelling system
                # for --mode train_test : we don't want the following block of code to run, so we put it inside the if condition
                # for --mode predict : we want it to run. However the mode (on checking set_trace) is actually val, so execute it without any if condition
                # if mode == 'predict':
                predictions = batch.labels[:,0,:]
                for i in range(word_log_probs.shape[0]):
                    for j in range(word_log_probs.shape[1]):
                        if batch.labels[i,0,j] != -100:
                            max_log_probs[i,j] = word_log_probs[i,j,batch.labels[i,0,j]]
                    # ipdb.set_trace()

                padding_labels = (batch.labels[:,0,:]!=-100).float()
                # sro_label_predictions =  padding_labels #(predictions!=0).float() *
    ...
    def evaluation_end(self, outputs, mode):
        ...
        if self.hparams.task == 'conj':
            ...
            if 'predict' in self.hparams.mode:
                ...
                # computing accuracy
                acc_b1=torch.mean(torch.mean((outputs[0]['predictions'][:,0,:] == outputs[0]['ground_truth'][:,0,:]).float(), dim=1)).item()
                acc_b2=torch.mean(torch.mean((outputs[1]['predictions'][:,0,:] == outputs[1]['ground_truth'][:,0,:]).float(), dim=1)).item()
                acc=(acc_b1+acc_b2)/2.0
                
                metrics['carb_f1'] = acc
                metrics['carb_auc'] = acc
                metrics['carb_lastf1'] = acc
            # ipdb.set_trace()
