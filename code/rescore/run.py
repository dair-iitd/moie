...
def predict(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader, all_sentences, mapping=None, conj_word_mapping=None):
    ...
    # '''
    # creating sentences in allennlp format 
    #     the sentences for prediction are in model.outputs[:]['meta_data'] (note- these are raw sentences i.e. not padded and of different seq len)
    #     the predicted labels are in model.outputs[:]['predictions'] (this has labels over fix seq len (upper bounded) but use labels only uptil the resp seq len)
    # '''
    # # ipdb.set_trace()
    # dict_1 = {'<arg1>': '</arg1>', '<rel>': '</rel>', '<arg2>': '</arg2>'}
    # dict_2 = {1: '<arg1>', 2: '<rel>', 3: '<arg2>'}
    # labelled_sents = []
    # for batch in model.outputs:
    #     sents = batch['meta_data']
    ...
    #         # insert labelled extractions <arg1>, etc
    #         prev_lbl = None
    #         lbld_sen = []
    #         tok_sen = sen.split(' ')
    #         for tok_id, tok in enumerate(tok_sen):
    #             tok_lbl = dict_2[preds[sen_id, 0, tok_id].item()]

    #             if prev_lbl == None:
    #                 lbld_sen.append(tok_lbl)
    #                 prev_lbl = tok_lbl
    #             elif tok_lbl != prev_lbl:
    #                 lbld_sen.append(dict_1[prev_lbl])
    #                 lbld_sen.append(tok_lbl)
    #                 prev_lbl = tok_lbl    
    ...
    #         lbld_sen.append(dict_1[prev_lbl])
    #         lbld_sen = ' '.join(lbld_sen)
    #         new_line += (lbld_sen + "\t")

    #         # insert confidence/score
    #         new_line += (str(scores[sen_id,0].item()) + "\n")

    #         # insert new_line in the final labelled sentences
    #         labelled_sents.append(new_line)
    #         # ipdb.set_trace()

    # out_file = open('en_mBERT_pred_hi.allennlp', 'a') 
    # out_file.writelines(labelled_sents)
    # out_file.close()
...
def splitpredict(hparams, checkpoint_callback, meta_data_vocab, train_dataloader, val_dataloader, test_dataloader, all_sentences):
    ...
    if hparams.rescoring:    
        ...
        # testing rescoring 
        inp_fp = '/home/shubham/shak2_shared/en_gen2oie_test.allennlp'
        ...
        for line_i, line in enumerate(rescored):
            ...
            try:
                ...
                arg2_temp = re.findall("<arg2>.*</arg2>", fields[1])
                arg2 = "" if len(arg2_temp)==0 else arg2_temp[0].strip('<arg2>').strip('</arg2>').strip()
            except IndexError:
                print(fields)
                print(line)
...
def main(hparams):
    ...
    if hparams.task == 'conj':
        ...
        # hparams.test_fp = 'data/en_gen2oie_test.allennlp.ext_sen_lbls' if hparams.test_fp == None else hparams.test_fp
        if hparams.debug:
