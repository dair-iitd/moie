...
Options:
  ...
  --language=LANGUAGE          Language of CARB evaluation
'''
...
class Benchmark:
    ...
    def compare(self, predicted, matchingFunc, output_fn, error_file = None, binary=False, strategy='sm'):
        ...
        for sent, goldExtractions in gold.items():            
            ...
            # ignore = False
            # for ge in goldExtractions:
            #     if ge.pred.strip() in ['DESC','ISA', '_', 'BIRTH', 'DEATH', 'IN'] \
            #     or ge.args[0].strip() == '_' \
            #     or ge.args[1].strip() == '_' \
            #     or '[' in ge.pred or ']' in ge.pred or 'X' in ge.pred or 'Y' in ge.pred or 'Z' in ge.pred \
            #     or '[' in ge.args[0] or ']' in ge.args[0] or 'X' in ge.args[0] or 'Y' in ge.args[0] or 'Z' in ge.args[0]\
            #     or '[' in ge.args[1] or ']' in ge.args[1] or 'X' in ge.args[1] or 'Y' in ge.args[1] or 'Z' in ge.args[1]:
            #         ignore = True
            #         break
            # if ignore:
            #     continue

            num_final_sentences += 1
...
if __name__ == '__main__':
    ...
    if args['--language']:
        import matcher
        matcher.language = args['--language']
