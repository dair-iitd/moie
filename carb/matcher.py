...
class Matcher:
    ...
    def linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        predicted_words = ex.pred.split()
        gold_words = ref.pred.split()
        # Make each word lower cased if ignoreCase is true
        if ignoreCase:
        ...
        forms_of_be = ["es","<Ser>","ser",'soy','están','estaba','fueron','estado']
        if "<Ser>" in predicted_words:
            for form in forms_of_be:
                if form in gold_words:
                    matching_words += 1
                    predicted_words.remove("<Ser>")
                    break
        forms_of_be = ["é","<Ser>","ser",'sou','estão','estava','estamos','fui','sendo']
        if "<Ser>" in predicted_words:
            for form in forms_of_be:
                if form in gold_words:
                    matching_words += 1
    ...
    def binary_linient_tuple_match(ref, ex, ignoreStopwords, ignoreCase):
        ...
        global language
        # if language == 'zh': 
        # e.args[0] = ' '.join(e.args[0]).strip(); e.args[1] = ' '.join(e.args[1]).strip(); e.pred = ' '.join(e.pred).strip()
        # r.args[0] = ' '.join(r.args[0]).strip(); r.args[1] = ' '.join(r.args[1]).strip(); r.pred = ' '.join(r.pred).strip()
        # if len(e.args) == 2:
        #     e.pred = e.args[0] +' '+ e.pred +' '+ e.args[1]
        #     e.args[0] = 'a0'; e.args[1] = 'a1'
        # else:
        #     e.pred = e.args[0] +' '+ e.pred
        #     e.args[0] = 'a0'
        # if len(r.args) == 2:
        #     r.pred = r.args[0] +' '+ r.pred +' '+ r.args[1]
        #     r.args[0] = 'a0'; r.args[1] = 'a1'
        # else:
        #     r.pred = r.args[0] +' '+ r.pred
        #     r.args[0] = 'a0'
